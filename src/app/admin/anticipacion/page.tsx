...
import { createAnticipacion, deleteAnticipacion } from "@/lib/actions/datos";
...
export default function AnticipacionPage() {
  ...
  const [data, setData] = useState<Anticipacion[]>([]);
  const [open, setOpen] = useState(false);
  const [form, setForm] = useState({ categoria: "", porcentaje: "" });

  const handleSelected = async (dId: string, pId: string) => {
    setDestinoId(dId);
    setPeriodoId(pId);
    const res = await fetch(`/api/datos/anticipacion?destinoId=${dId}&periodoId=${pId}`);
    setData(await res.json());
  };

  const handleCreate = async () => {
    if (!form.categoria.trim() || !form.porcentaje) return;
    await createAnticipacion({ destinoId, periodoId, categoria: form.categoria.trim(), porcentaje: parseFloat(form.porcentaje) });
    setOpen(false);
    setForm({ categoria: "", porcentaje: "" });
    handleSelected(destinoId, periodoId);
  };

  const handleDelete = async (id: string) => {
    if (confirm("¿Eliminar este registro?")) {
      await deleteAnticipacion(id);
      handleSelected(destinoId, periodoId);
    }
  };
  ...
  return (
    <div className="space-y-6">
      <h2 className="text-2xl font-bold">Anticipación de Viaje</h2>
            ...
            <Dialog open={open} onOpenChange={setOpen}>
              ...
              <DialogContent>
                ...
                <div className="space-y-4">
                  <Input placeholder="Categoría (ej: Menos de 1 semana)" value={form.categoria} onChange={(e) => setForm({ ...form, categoria: e.target.value })} />
