...
import { createCompaniaViaje, deleteCompaniaViaje } from "@/lib/actions/datos";
...
export default function CompaniaPage() {
  ...
  const [data, setData] = useState<Compania[]>([]);
  const [open, setOpen] = useState(false);
  const [form, setForm] = useState({ tipo: "", porcentaje: "" });

  const handleSelected = async (dId: string, pId: string) => {
    setDestinoId(dId);
    setPeriodoId(pId);
    const res = await fetch(`/api/datos/compania?destinoId=${dId}&periodoId=${pId}`);
    setData(await res.json());
  };

  const handleCreate = async () => {
    if (!form.tipo.trim() || !form.porcentaje) return;
    await createCompaniaViaje({ destinoId, periodoId, tipo: form.tipo.trim(), porcentaje: parseFloat(form.porcentaje) });
    setOpen(false);
    setForm({ tipo: "", porcentaje: "" });
    handleSelected(destinoId, periodoId);
  };

  const handleDelete = async (id: string) => {
    if (confirm("¿Eliminar este registro?")) {
      await deleteCompaniaViaje(id);
      handleSelected(destinoId, periodoId);
    }
  };
  ...
  return (
    <div className="space-y-6">
      <h2 className="text-2xl font-bold">Compañía de Viaje</h2>
            ...
            <Dialog open={open} onOpenChange={setOpen}>
              ...
              <DialogContent>
                ...
                <div className="space-y-4">
                  <Input placeholder="Tipo (ej: Familia, Pareja)" value={form.tipo} onChange={(e) => setForm({ ...form, tipo: e.target.value })} />
